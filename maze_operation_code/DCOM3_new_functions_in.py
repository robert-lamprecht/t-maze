...
def initialize():
    ...
    while True:
        ...
        if MazeTexture not in ['R/S','I/S','C','G', 'S']:
            print('not a texture')
        else:
            break
...
def runTrial (savefile, numTrials, rewardBeforeLick=False,  valveOpenTime=50, lickUntilCorrect=False, biasCorrection=False):
    ...
    if biasCorrection: 
        try:
            with open("D:\TMAZE_DATA\data_BEB5_20240710_164546.json", 'r') as file:
                data = json.load(file)
                dataLoaded = True
            if mouseID in data:
                list_of_licks = []
    ...
    for trial in range(trials):
        ...
        if mouseID not in data: 
            ...
            data[mouseID][trial_key] = {'MazeTexture':MazeTexture,'rewardBefLick': rewardBeforeLick,'lickUntilCorrect': lickUntilCorrect,'decision': decision, 'floorID': current_floor,
                                        ...
                                        'right_probability': rightProb,'rewardAmount(ms)': valveOpenTime,'BiasCorrection(T/F)':biasCorrection}
